...
import { SystemRenderModel } from "src/app/shared/models/progress-tab/progress-tab.model";
declare var $: any;

@Component({
  selector: "directive-previewer-system",
  templateUrl: "./directive-previewer-system.component.html",
  styleUrls: ["./directive-previewer-system.component.scss"],
})
export class DirectivePreviewerSystemComponent implements OnInit, OnChanges {
  @Input() data2Drawing: SystemRenderModel = null;
  ...
  updateTableView() {
    setTimeout(() => {
      ...
      if (_scale !== 1) {
        ...
        setWidth(".system-report", _pixel4Each);
        setWidth(".punch-system-report", Math.floor(_pixel4Each * 0.33));
        setWidth(".itrs-system-report", Math.floor(_pixel4Each * 0.35));
        setWidth(".handover-system-report", Math.floor(_pixel4Each * 0.65));
        setWidth(".completion-system-report", _pixel4Each);
      }
