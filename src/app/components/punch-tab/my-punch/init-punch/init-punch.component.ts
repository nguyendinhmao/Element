import { Component, ViewChild, Input, Output, OnChanges, SimpleChanges, EventEmitter, ElementRef, AfterViewInit } from "@angular/core";
...
import { PunchPageListModel, PunchPageSearchModel, DrawingLookUpModel, Signature, LookUpFilterModel, ImageLookUp } from "src/app/shared/models/punch-page/punch-page.model";
...
@Component({
  selector: "init-punch",
  styleUrls: ["./init-punch.component.scss"],
  templateUrl: "./init-punch.component.html",
})

export class InitPunchComponent implements OnChanges, AfterViewInit {
  //--- Input & Output
  ...
  @ViewChild("initPunch") initPunch: ElementRef;
  ...
  defaultComponentName: string = "Create";
  totalItems: number = 0;
  ...
  executeGetPunchDataOnline(pageNumber?: number, pageSize?: number, sortExpression?: string, punchNo?: string) {
    this.punchPageService.getUnSubmittedPunches(
      this.projectKey,
      this.tagNo || '',
  ...
  executeGetPunchDataOffline(sortExpression?: string) {
    ...
    this.idbService.getAllData(this._storeName).then((res) => {
      if (res && res.length > 0) {
        const _unSubmittedPunches = res.filter(_punch => !_punch.isDeleted && (_punch.status === PunchStatuses.draft || _punch.status === PunchStatuses.rejected));

        const _filterSystem = this.filterOffline(_unSubmittedPunches, 'systemNo', this.punchPageSearchModel.systemNo),
  ...
  assignPunchData2Model(punches) {
    ...
    this.initPunch.nativeElement.style.minHeight = "auto";
  ...
  onSuccessEditModal = (punchItem: PunchPageListModel) => {
    if ((punchItem && punchItem.punchId) || this.isOffline) {
      this.onRefreshData();
      this.onGetCounter.emit(this.defaultComponentName);
    }
  ...
  toggleDropdown = () => {
    ...
    if (this.isToggleDropdown && dropdownHeight > 0) {
      this.initPunch.nativeElement.style.minHeight = dropdownHeight + "px";
    } else {
      this.initPunch.nativeElement.style.minHeight = "auto";
    }
  };
