...
import { CreateTagNoModel, DrawingLookUpModel2, TagLookUpModel } from 'src/app/shared/models/data-tab/data-tagno.model';
...
import { DrawingLookUpModel, LoadingSelectionPunchModel, SelectionControlName } from 'src/app/shared/models/punch-page/punch-page.model';
import { DataDrawingService } from 'src/app/shared/services/api/data-tab/data-drawing.service';

@Component({
  selector: 'create-data-tagno',
  styleUrls: ["./create-data-tagno.component.scss"],
  templateUrl: './create-data-tagno.component.html'
})

export class CreateDataTagNoComponent implements OnInit {
  @Input() visible: boolean = false;
  @Input() projectKey
  @Output() onSuccess: EventEmitter<boolean> = new EventEmitter();
  @Input() projectId: string;

  creationModel: CreateTagNoModel = new CreateTagNoModel()
  ...
  onGetDrawingLookUp = (projectKey: string) => {
    return new Promise((resolve, reject) => {
      this.dataDrawingService.getDrawingLookUp(projectKey).subscribe(
        (res) => {
  ...
  onSaveData = (form: NgForm) => {
    ...
    let tagnoCreateModel = <CreateTagNoModel>{
      ...this.creationModel
    };
    this.datatagnoService.createTagNo(tagnoCreateModel).subscribe({
      complete: () => {
        this.clientState.isBusy = false;
  ...
  onGetLookUpTag() {
    this.clientState.isBusy = true;
    this.datatagnoService.getTagLookUp(this.projectKey).subscribe(res => {
  ...
  onGetSubSystemBySystem(event) {
    ...
    this.creationModel.subSystemId = null;
  }
  onCancel = () => {
  ...
  onClearDrawingSelect = (drawing: DrawingLookUpModel2) => {
    if (
      drawing &&
      drawing.id &&
      this.creationModel.drawingIds &&
      this.creationModel.drawingIds.length > 0
    ) {
      this.creationModel.drawingIds = this.creationModel.drawingIds.filter(
        (id) => id != drawing.id
      );
    }
  };
